...
`boundary_disputed`, starts `boundary_2` at zoom 5 without the Pakistan-China line, adds the bundled
...
only as disputed lines (rule 1 hides them) and the overlay's `claim` outline draws India's boundary there. The same five
rules are specified for Android (lead's task of 2026-09-24, Android team); see its README for its status.
