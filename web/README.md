...
### The live UI test after a merge that changes the web app

Owner rule of 2026-09-24, refined the same day: after a merge to `main` that runs the `Web` deploy (a change under
`web.yml`'s path filter: `web/**`, `.github/workflows/web.yml`, `.github/firebase-tools/**`), once the deploy has
finished, the live site is tested in detail with `tools/live-ui` ([docs/06](../docs/06-test-plan.md) TC-M-26). An
Android-only or docs-only merge skips it. It is not a workflow; the lead session runs it and reports the counts.
...
| 2026-09-24 | **The live UI test only after a merge that changes the web app** (owner rule of 2026-09-24, refined the same day: "Do the web app testing only if the web app changes"; [docs/06](../docs/06-test-plan.md) TC-M-26). The *Test* subsection now runs it only after a merge to `main` that runs the `Web` deploy (`web.yml` path filter); an Android-only or docs-only merge skips it. README only, no code change. |
| 2026-09-24 | **Web backlog fixes (S4b-BL-1, -2, -6, -7; the web halves of S4b-BL-13 and -20; branch `claude/doorprints-dev-continue-fzcge2`, owner request of 2026-09-24).** **S4b-BL-1 (`data/sync.service.ts`):** `lastError` is cleared only when a run succeeds, no longer when one starts. `failureRun` goes up only for a failed run the user asked for (*Sync now*, *Try again*, *Download now*) or one whose message differs from the one on screen (`recordFailure`, `sameMsg`), and a background run failing the same way leaves `lastErrorForced` as it was. So the polite "Automatic sync failed" note is no longer removed and read again on every 30-minute or after-edit pass, and a failure the user saw does not move to the quiet line. While `sync.running()`, Your data's failure card stays in `.refresh-slot.stale` with a *Syncing…* (`data.syncing`) refresh bar, and the first-run banner's failure line stays with its sign dimmed and a *Downloading…* bar (`app-banners.ts`, `.failure-slot`). **S4b-BL-2 (house page):** `persist()`, *Use my location*, *Fill in the form* (listing) and *Fill address from map* no longer clear their card when they start. The card stays, dimmed in `.refresh-slot.stale` with a bar named *Saving…*, *Finding your location…*, *Reading the listing…* or *Looking up…*, and the run's end removes it or puts the new failure in its place. **S4b-BL-6 (`styles.css`):** one rule, `input[aria-invalid='true'], textarea[aria-invalid='true']`: a border in `--error-text` plus a 1px inset ring of the same colour (2px drawn, no layout shift), kept while focused. Contrast (WCAG 1.4.11): light `#B3261E` is 6.54:1 on `--surface` `#FFFFFF` and 6.02:1 on `--bg` `#F4F6F5`; dark `#FF8E86` is 7.40:1 on `#19211F` and 8.25:1 on `#101614`. **S4b-BL-7:** new TestBed specs. `pages/plan/plan-page.spec.ts` (4 cases): a typed start; a field made invalid, then corrected; a location failure withdrawn by a map click; the `aria-describedby` and `aria-invalid` of *Choose a start point first*. maplibre-gl is replaced by a stand-in map through `vi.mock` of the package (the Angular builder refuses `vi.mock` of relative imports). `pages/house-detail/house-detail-page.spec.ts`: the two `startWithoutPosition()` guard cases (`/houses/new` with no position, page destroyed before `houses()` resolves, and before it rejects: no draft, no *Draft restored*, no title), plus 4 S4b-BL-2 cases in a separate `describe`, one per kept card. **S4b-BL-20, web half (reset server):** the API has no "highest syncVersion" call, and a pull cannot tell (`?since=<cursor>` answers `[]` from a reset server and an unchanged one alike). The signal is the answer to a pushed house or visit: every accepted write takes `nextval('sync_seq')` (one sequence for all three tables, under the advisory lock, backend `SyncVersions`). When the server **accepted** the write (the answer's `updatedAt` is not later than the one sent) and gave it a `syncVersion` at or below the highest stored cursor, the server's change log is behind this browser (`serverWasReset`). The run then marks every house and visit dirty and every stored photo not uploaded (`LocalStore.markAllForResync`), sets the three cursors to 0, pushes everything again, pulls from 0, and says so: `data.serverReset` through the announcer, and a line on Your data's sync card. A kept newer row (last write wins, old version) and "Remove all my data" on the server (the sequence carries on) are not taken for a reset. **Limit:** the reset is seen at the first push after it, so a browser with nothing to send keeps pulling from its old cursors until its next edit. Detecting it without a push needs a server change (for example the highest version in `GET /api/stats`), handed back as a backlog item. The fake API in `sync.service.spec.ts` now answers a push with the next version of a sequence, as the backend does. **S4b-BL-13, web half:** comment-only. `india-boundaries.ts` (rule 2 of the header, `TILE_ZOOM_GUARD`, `takesTileZoomGuard`) and this README's intro now say that maplibre-gl **and** maplibre-native skip a minzoom 5 layer in a zoom 0-4 tile, so the guard is defence in depth on both apps. New string `data.serverReset` in en/hi/ta/te (hi, ta and te under review). Tests: 492 pass (19 new); `ng build` passes. |
| 2026-09-24 | **Phone display fixes (owner report of 2026-09-24, "display issues when opened on a mobile browser", with a screenshot of the Map page on a 384px Android phone; branch `claude/doorprints-dev-continue-fzcge2`).** Checked on emulated Pixel 7, Galaxy S9+, iPhone SE and iPhone 14 (touch, pixel ratio, mobile viewport), 320, 360 and 384px wide, landscape, 130-200% text, en/hi/ta/te, both themes, every route and a house page, and the on-screen keyboard. **Map page (`map-page.css`, `map-page.html`, `map-page.ts`, new `list-peek.ts`, `shared/map-style.ts`):** MapLibre opens compact credits on load and folds them only on a one-finger drag, which cooperative gestures never deliver on a phone, so the two-line credits stayed across the map for good; `foldAttribution` now folds them into the (i) button 5 s after the style first loads (`ATTRIBUTION_SHOW_MS`) or at the first user zoom or move, on maps up to 640px wide (desktop unchanged; the (i) opens them in full). The map was `55vh`, 55% of the viewport *without* the address bar, so on the owner's phone it took 391px and the counters' captions sat under the bottom bar; it now fills the visible page area (`100cqh` of `<main>`, which is a size container, app.ts) less the list's heading and counters, measured into `--map-peek` (`listPeek`), so "Your houses" and every counter show under it before any scrolling; at least 256px, and never so short that MapLibre's 196px control column leaves the top of the map above the legend row (the controls win where both do not fit: Tamil at 130% on 384x615, 320px at 150%; the counters then start below the bar). Up to 384px the counters are one line each ("0 Houses"), about 40% shorter, a long caption wrapping inside its cell. `:host` grows with the list on phones: at `height: 100%` the list overflowed it, and the empty list's "Add house" stayed 40px under the bottom bar at the end of the page. The legend moves to its own row above the actions when its widest item no longer fits beside them (320px at 150% text ran "Shortlisted" under "Add house"), and the actions keep their one-line width while the legend shows. **Shell (`app.ts`):** the phone header is 48px (was 56); the brand link is at least 44px wide (it was 28x44 wherever its name is hidden, phones and 601-900px); the bottom bar's columns are `minmax(0, 1fr)` and the skip link is no wider than the screen (at 200% Tamil text each made a 360px phone lay the page out 438px wide and zoom out); the skip link is hidden by its own height (`translateY(-110%)`), not a fixed 100px, which its wrapped Tamil label outgrew at 130% (31px of it showed at the top of every page). **Long words at 200% text:** `overflow-wrap: break-word` on `body`, and `anywhere` on `.btn`, `.chip`, `.option > span` and `.checkbox > label` up to 600px; `.checkbox` and Your data's format grid use `minmax(0, 1fr)`, the format and Ask filter grids `minmax(min(…, 100%), 1fr)`, Your data's count pills can shrink: a Tamil word wider than the screen had scrolled pages 32-103px sideways. With a text field focused on a phone shorter than 500px (the keyboard where the browser shrinks the page, or landscape) the bottom bar steps aside instead of covering the field. **Banners (`app-banners.ts`):** at most 40% of the visible height, scrolling inside (the iPhone install offer in Tamil took 255 of 568px, and left the house form 1px with the keyboard open). **House page:** the phone toolbar title takes up to two lines before its ellipsis (320px cut "New house" and புதிய வீடு), its actions at most 60% of the bar; the photo viewer uses `dvh`. **Checks:** `tools/live-ui/live-ui.js` has a `mobile` pass (`ONLY=mobile` runs it alone; the owner's 384x615 at 130% text is one of its phones): sideways scroll of the page and of `<main>`, a page laid out wider than the screen, touch targets under 44px, fields under 16px text, anything under the bottom bar at the end of a page, map controls outside the map or over its legend and buttons, the credits folded after 5 s, the map's labels drawn (glyphs downloaded and the names' dark pixels in the map), the heading and counters above the bottom bar (unless the map is at its control-column minimum), the skip link hidden until focused, and the focused field above the keyboard. New specs `shared/map-style.spec.ts` (4) and `pages/map/list-peek.spec.ts` (4). Tests: 514 pass; `ng build` passes. |
| 2026-09-24 | **Held areas, hand-overs and the reset-server stats (S4b-BL-12, S4b-BL-17, S4b-BL-20 completed; branch `claude/doorprints-dev-continue-fzcge2`, PR #24; [docs/10](../docs/10-sprint-log.md) §13.9).** **S4b-BL-12:** from tile zoom 9 the tiles draw Pakistan's district and tehsil lines and China's county lines inside India's outline as undisputed admin 5-6 lines with no country code; `india-boundaries.ts` step 2c ANDs `boundary_3`'s filter with `heldAreasRule(HELD_AREAS)`, `["!", ["within", P]]`, P the one Polygon of the new `public/geo/in-held-areas.geojson` (12 575 bytes, sha256 `8fa2db12…80c3`, byte-identical to Android's asset, built by `scripts/geo/build_in_held_areas.py`), imported as text (`with { loader: 'text' }`, `src/geojson-text.d.ts`) because the rule is needed synchronously at `style.load` (the file is also served and precached, unused at run time); a missing or malformed polygon, or a deprecated-syntax `boundary_3` filter, is a warning. Left: a few merged pieces at zoom 9-11 near the LoC; foreign admin lines within about 20 km outside the outline next to the held areas are not drawn. **S4b-BL-17:** `scripts/geo/find_shared_stretches.py` gained a `tidy()` step; `in-boundaries.geojson` rebuilt (sha256 `c3cdf5fb…f63f`): no loop at Sikkim's north-west tri-junction, no overrun at Jomotsangkha or Longwa; Doklam unchanged on purpose. **S4b-BL-20:** `StatsDto.maxSyncVersion`; `SyncService.pass()` asks `api.stats()` (through `call`, so 429 handling and cancellation apply) once a cursor is above 0 and resets before the push when `serverBehind`; a failed or field-less answer is unknown, and the push-answer check stays for older servers, so the limit in the row above (a browser with nothing to send) is gone. Tests: `india-boundaries.spec.ts` +9 (MapLibre's own `featureFilter` from `@maplibre/maplibre-gl-style-spec`), `sync.service.spec.ts` +5; with the phone fixes 41 files and 514 tests pass. |
