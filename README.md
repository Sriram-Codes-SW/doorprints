...
"Natural Earth" in the map attribution anyway. Both apps apply the same rules. Where the base map's tiles draw India's border themselves
(for example the Himachal Pradesh and Uttarakhand border, Sikkim and the Wakhan), the outline is drawn only in the
country view and the tiles' more precise line takes over from zoom 5, so the border is one line; the Assam-Arunachal
Pradesh state line, which the tiles leave undrawn, is drawn from zoom 5 from the same file (branch
`fix/india-boundary-lines`, not yet deployed). Known limits: the outline is typically 1.5-3 km off the true line, up
to about 5 km in a few mountain stretches, visible only when zoomed into the Himalaya; at street zoom the hand-over to
the tiles' line shows as a small step; and while closer tiles load, or offline without them, those stretches show no
line from zoom 5 (ADR-22 *Consequences*).
...
| 2026-09-24 | *Map data and credits*: one line from zoom 5 where the base map draws India's border itself (the second, close line is gone) and the Assam-Arunachal Pradesh state line is drawn from zoom 5 (branch `fix/india-boundary-lines`, [docs/10](docs/10-sprint-log.md) §12.10); the known limits restated (1.5-3 km, up to about 5 km in a few mountain stretches; the hand-over step; no line on those stretches from zoom 5 while tiles load or offline). |
