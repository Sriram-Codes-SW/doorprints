...
/**
 * Read-only tools exposed to MCP clients (Claude Desktop / Cowork). Nothing here writes. {@code askHouseHunt}
...
public class McpHouseTools {
    ...
    @Tool(name = "askHouseHunt", description = """
    ...
    public AskResponse askHouseHunt(@ToolParam(description = "The question") String question) {
        var service = rag.getIfAvailable();
        if (service == null) {
            return new AskResponse("askHouseHunt is unavailable: AI features are disabled on this server "
