...
S4b-BL-28 in v0.49, from CMP-4 P4b (§13.7).
...
(`Character.isSpaceChar` and `isISOControl` rewritten on `Char.category`). One deliberate difference from OpenJDK: the
...
the fixed cases and on 300 000 generated and mutated strings (fixed seeds), for the JVM rule, and for the app's rule
on every string without a `_`.
...
has no automated test), S4b-BL-28 (`SyncHealthTest` and `ExportGrantsTest` still JUnit in `:app`).
