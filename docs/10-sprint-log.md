...
and its design review; S4b-BL-35 to S4b-BL-50 in v0.51, from the combined CMP-5..7 change (§13.9), the owner's
Vulkan question, the emulator's map labels, the phone display fixes and the S4b-BL-10 finding.

| # | Client | Ticket | Fix | Owner |
|---|---|---|---|---|
| ~~S4b-BL-1~~ | Web | ~~**`sync.service.ts` `lastError`** (raised three times). `runOnce()` sets `lastError` to null at the start of every run, so the forced failure card and the first-run banner disappear during *Sync now* or *Try again* and the content below jumps. The polite "Automatic sync failed" note is removed and read again on every failed background pass (every 30 min, and 3 s after an edit). `failureRun` also goes up on runs the user did not ask for~~ **Done** (PR #24, branch `claude/doorprints-dev-continue-fzcge2`; §13.9): `lastError` cleared only on success, `failureRun` raised only for a forced run or a new message, the card in `.refresh-slot.stale` with a *Syncing…* bar, the banner line kept ([05](05-ux-accessibility-i18n.md) §5.1; [06](06-test-plan.md) TC-U-75) | Clear `lastError` only on success. Bump `failureRun` only when the run is forced or the message changed. Render the card in `.refresh-slot.stale` while `sync.running()`, with a "Syncing…" refresh bar | Web |
| ~~S4b-BL-2~~ | Web | ~~**House page retry cards** (raised twice). `persist()`, `useMyLocation()`, the listing fill and the address lookup clear their card before the run, so the form jumps on a retry~~ **Done** (PR #24, branch `claude/doorprints-dev-continue-fzcge2`; §13.9): the save, listing-fill and location or lookup cards stay, dimmed, with a named bar, until the run ends; the scope is in [05](05-ux-accessibility-i18n.md) §5.1 (TC-U-75) | Keep the `RunResult` card, dimmed with `.refresh-slot.stale`, while saving, filling, geocoding or locating runs, and replace it when the run ends. Docs then records the scope in [05](05-ux-accessibility-i18n.md) §5 and §5.1 | Web, then Docs |
...
| S4b-BL-9 | Both | **Re-check India's boundaries after each OpenFreeMap planet or style update** (owner issue P0 of 2026-09-24, §12.8; new id). The rules name Liberty's layers (`boundary_disputed`, `boundary_2`, `label_state`) and rely on what the tiles carry (disputed lines flagged, `adm0_l`/`adm0_r` from zoom 5, the two state names); a renamed layer only gives a warning, and a new disputed layer or a new name would show on the map **Checked 2026-09-24** (PR #24): the OpenFreeMap planet is still `20260913_164504_pt`, Liberty's 18 fixture layers are JSON-identical (same order, sources and sprite, no new `boundary` or `place` layer), and `find_shared_stretches.py` on a `--no-shared` build gives the same 7 stretches digit for digit, so nothing needed a rebuild. Stays open (recurring); since S4b-BL-12 each check also re-runs `web/scripts/geo/build_in_held_areas.py --check` and, on a new planet, rebuilds `in-held-areas.geojson` (new sha256 in `IndiaBoundaryDataTest` and the web spec) | Run [06](06-test-plan.md) TC-M-25 after each update OpenFreeMap announces and before each release; if a warning appears or a line shows, update both apps' rules together (`india-boundaries.ts`, `IndiaViewRules.kt`) and their tests. Later: a screenshot test ([06](06-test-plan.md) §10). Since branch `fix/india-boundary-lines` (§12.10), also re-run `web/scripts/geo/find_shared_stretches.py` on the new planet (on a build made with `build_in_boundaries.py --no-shared`), paste its output into `SHARED` in `build_in_boundaries.py`, rebuild both copies of `in-boundaries.geojson` and update the sha256 in `IndiaBoundaryDataTest`: a moved or newly disputed tile line would otherwise leave a gap or a doubled line | Lead, with Web and Android |
| S4b-BL-10 | Both | **Consider a Survey of India-derived outline** (§12.8; new id). The claim outline is Natural Earth 1:10m, a median of about 1.55 km (Jammu-Sialkot) and 1.6 km (McMahon line) off the true line, 3.9 km at the 90th percentile (Web team's tile decode; "about a kilometre" until v0.33) ([02](02-threat-model.md) RR-16) **Finding of 2026-09-24** (PR #24; sources in [02](02-threat-model.md) RR-16): no Survey of India outline file was found whose licence clearly allows redistribution in an AGPL app. The SoI portal lists its boundary data at no charge but states no licence; SoI's Digital Licence excludes value-added products, incorporation in a third-party product and publication or free distribution without written permission; the Geospatial Guidelines 2021 allow display and printing only; no SoI outline under GODL-India was found; datameet's `india-soi.geojson` is not cleared by SoI. **Natural Earth stays.** Owner options: ask SoI for written permission (the portal FAQ names the contact), or S4b-BL-50 (an accuracy check of a CC-0, non-SoI source). Open: owner decision | Only if its licence clearly allows redistribution in an open-source app and its web build at zero cost; then rebuild `in-boundaries.geojson` with `web/scripts/geo/build_in_boundaries.py` (or a successor), update the sha256 in `IndiaBoundaryDataTest`, re-run TC-M-25 | Lead (owner decision needed on the source) |
...
| ~~S4b-BL-20~~ | Both | ~~**Clients detect a reset server** (review of PR #19, [08](08-operations-runbook.md) §11; new id). Clients push only rows changed since their last sync and pull from a stored cursor that is reset only when the server's address changes, so a server whose database was replaced (a new, empty database; a restore from an older dump) silently loses the devices' older rows and hides changes below the cursors.~~ **Done** (PR #24, branch `claude/doorprints-dev-continue-fzcge2`; §13.9), `a9c28e8`, `0989550`: `GET /api/stats` returns `maxSyncVersion` (`sync_seq`'s position; TC-I-36); Android and the web reset when it is below a cursor, or when an accepted push is answered at or below one (older servers): every row marked for upload, cursors 0, push and pull everything, and say so (web announces; Android shows it in the sync outcome line, S4b-BL-45) ([03](03-design.md) §10.1, [08](08-operations-runbook.md) §11.1, [02](02-threat-model.md) RR-17; TC-U-74) | When the server's highest `syncVersion` is below a stored cursor, reset the cursors and mark every local row dirty (a full re-push and re-pull), and say so to the user; Android and web alike, with tests | Android, Web |
...
| ~~S4b-BL-23~~ | Android | ~~**The `Repository` uses Room calls that exist only on Android** (found in CMP-4 P4a; new id). `db.withTransaction { }` (two places) and `db.invalidationTracker.createFlow("houses", "visits", "photos")` are Room's Android API; the database itself is common since P4a (§13.6). Left in `:app` with the `Repository`, which moves in P4c~~ **Done in CMP-4 P4c** (§13.8): `withImmediateTransaction` and `localTablesChanged` in `:shared` commonMain; `RepositoryTransactionTest` (TC-U-66); the import and undo tests pass unchanged. The flow call was already Room's common `createFlow`; only the transactions were Android API. | In P4c, write them with Room's common API (`useWriterConnection { it.immediateTransaction { } }` and the common `InvalidationTracker.createFlow`), with the import and undo tests (TC-U-52, `CopyUndoTest`) passing unchanged | Android |
...
| S4b-BL-36 | Android | **The iOS side of the CMP-5 seams is stubs** (CMP-5; new id). `PermissionRequests.ios` and the other iOS actuals compile but do nothing | With the iOS shell (CMP-8): real permission requests and settings links on iOS | Android |
...
| S4b-BL-44 | Android | **The Map has no JVM screenshot** (CMP-7; new id). Its chrome now renders in inspection mode around an empty map (as in `RootNavigationTest`) | Record a Map chrome set (4 languages x 2 themes, new reference images; CMP-0-BL-4 related) in a phase that re-records | Android |
| S4b-BL-45 | Android | **A reset server is not announced on Android** (S4b-BL-20; new id). The reset is said only in the stored sync outcome (Settings' server status line, the house list's warning reason when shown); a background sync gives no notification or snackbar, while the web announces it | Decide whether a one-time notice is wanted, then add it in four languages | Android, Design |
| S4b-BL-46 | Android | **Other Compose tests still use the unconfined test dispatcher** (S4b-BL-42; new id): `RootNavigationTest` (it avoids the visit-id path) and the instrumented `SmokeTest` (`createEmptyComposeRule`; production `currentLocation` hops to `Dispatchers.Main.immediate` because of it) | Move them to a `StandardTestDispatcher`, or record why not | Android |
| S4b-BL-47 | Android | **A Vulkan build of the map** (owner discussion of 2026-09-24; new id). The APK uses MapLibre's OpenGL ES build, because the default Vulkan build crashes without a Vulkan GPU (API 26 emulator, older phones); both cannot be in one APK (same class and native library names, no fallback) | At a Play release, if wanted: a second APK variant with `android-sdk`, routed by Play's multi-APK on `android.hardware.vulkan.version`; iOS uses Metal, no split ([03](03-design.md) ADR-22) | Android, DevSecOps |
| S4b-BL-48 | Android | **The emulator lays out the map's labels but does not draw them** (new id). On the CI emulator's SwiftShader GPU `queryRenderedFeatures` finds 49 placed labels at zoom 4 over India, but none shows (maplibre-native #3939, #3617, open); not an app bug, so emulator map screenshots show no names | Check the labels on a real device in the owner's TC-M-25 run after CMP-8 (Indic labels are TC-M-23 (d)); revisit when maplibre-native fixes it | Owner, QA |
| S4b-BL-49 | Web | **The map counters under the bottom bar on the smallest phone setups** (phone display fixes; new id). Where the map is at MapLibre's control-column minimum (Tamil at 130 % on 384 x 615, 320 px at 150 %) the list's heading and counters start below the bar | Move the legend to the top of the map on short phones, which would leave room for both; check with `tools/live-ui`'s mobile pass | Web, Design |
| S4b-BL-50 | Both | **Check a CC-0 outline's accuracy** (S4b-BL-10 finding; new id). datameet's `india-composite.geojson` (CC-0, built from the US LSIB and other open data, not SoI) might be closer to the true line than Natural Earth | Decode it against the tiles the way RR-16's figures were measured; only if clearly better, rebuild both boundary files and re-run TC-M-25 | Lead (owner decision first) |
...
- **Data.** `in-boundaries.geojson`, both copies byte-identical: sha256 `25984afa…a024` (was `700646ea…4954` on
  ...
  `web.yml` `pwa-files` requires the kinds `world`, `claim` and `state`. Since S4b-BL-17 (PR #24, §13.9) the file
  is 415 606 bytes, sha256 `c3cdf5fb…f63f`, `claim` 1 662 points; the Sikkim north-west loop and the overruns at
  Jomotsangkha and Longwa below are gone, the Doklam loop stays on purpose.
...
| CMP-4 | P4a, P4b, P4c | **Data in common.** P4a: Room KMP (the catalog's version, 2.8.5 since Dependabot #12) in `:shared`, keeping the db v2 identity hash (`RoomSchemaTest`) and adding a migration test. P4b: DataStore KMP, a `SecretStore` interface (Android Keystore, later iOS Keychain) and `ServerUrl` in common. P4c: a `Repository` interface in common; `CompareScreen` and `HouseFormRules` move | Upgraded installs open their data unchanged; settings and the saved key survive; `ServerUrlTest` passes on the common parser | Android | **Done** (P4a, P4b and P4c merged). **P4a merged** (PR #21, `e480b83`; §13.6): `AppDatabase`, entities, DAOs, `Converters` and `MIGRATION_1_2` in `:shared` commonMain, identity hash and `doorprints.db` unchanged, new `AppDatabaseMigrationTest` (TC-U-63). **P4b merged** (PR #22, `aa8f73a`; §13.7): `SettingsStore` on `datastore-preferences-core`, the `SecretStore` interface and `ServerUrl` in `:shared` commonMain; file and key names unchanged (`SettingsUpgradeTest`, TC-U-64); `ServerUrlTest` passes on the common parser. **P4c merged** (§13.8; PR #23, `f8dd6f9`): a `Repository` interface in `:shared` (`AndroidRepository` in `:app`), its transactions and change flow on Room's common API (S4b-BL-23, `RepositoryTransactionTest`, TC-U-66), the mappers common; `CompareScreen` and `HouseFormRules` in `:ui` commonMain; S4b-BL-28 done |
| CMP-5 | P5 | **Navigation and view models.** JetBrains navigation-compose 2.9.2 and lifecycle 2.11.0; ViewModels with injected dependencies; HouseList, Assistant, Settings, NotifyAsk and LocationPermission move | Deep links and Back behave as before; the moved screens pass the UI self-check | Android | **Done in code** (PR #24, open, the owner merges; §13.9), with CMP-6 and CMP-7 as one combined change (owner, 2026-09-24): `84fa735`, `13091ac`; `RootNavigationTest` (TC-U-68), the 64 screenshots unchanged |
| CMP-6 | P6a, P6b | **Edit, export and import.** P6a: `HouseEditScreen`, with the photo picker and camera behind a seam. P6b: the Export and Import screens and `ImportViewModel`; the workers behind an interface | Photos, copies and imports work as before (TC-U-52 and the Sprint 4a export and import cases) | Android | **Done in code** (PR #24; §13.9): P6a `f7e9ecd`, P6b `7e73411`, `cff4a63`; the export and import tests pass, the 64 screenshots unchanged |
| CMP-7 | P7 | **Map.** The common `MapScreen` chrome and `expect PlatformMap`: on Android the existing MapLibre `MapView` in `AndroidView`, on iOS `UIKitView` around `MLNMapView` from Swift. India's boundary logic lifted into a common `applyIndiaView(ops: StyleOps)`, with `IndiaViewOpsTest` in `commonTest` | **TC-M-25 re-run** and passed (ADR-22); the map looks and behaves as before | Android, Docs | **Done in code** (PR #24; §13.9): `e2b163c`; `IndiaViewOpsTest` (TC-U-73), the Map smoke test on API 26, 34 and 36 (TC-I-35). The iOS map view is CMP-8 (S4b-BL-43); the TC-M-25 device run is the owner's, after CMP-8 |
...
- **`Repository` interface** (`:shared` commonMain, `data/Repository.kt`): the members whose types are
  platform-neutral (the flows, reads and writes, sync, AI calls, the copy's reads, `applyImport`, `undoCopyImport`)
  and the result types (`LocalRows`, `LocalVersions`, `ImportResult`, `UndoResult`, `StreetInfo`, `AddPhotoResult`,
  moved unchanged). `:app`'s class is now `AndroidRepository : Repository` (`data/AndroidRepository.kt`, a rename;
  bodies unchanged but for `override` and the two transactions). `addPhoto(Uri)`, `photoDir()` and `photoFile(id)`
  stay Android-only.
- **Why an interface, not a moved class.** The class also holds the photo files (`java.io.File`), decoding a photo
  (`Bitmap`, `ExifInterface`, a `Uri`), WorkManager's "sync soon" and the app-wide `HttpClient`. Moving it meant a
  seam for each of those and a rewrite of the sync and import code; the interface keeps every query and write where it
  was (the smaller, safer diff). The rest is **S4b-BL-32**.
- **Room's common API** (S4b-BL-23; `data/DatabaseTransactions.kt` in `:shared` commonMain):
  `withImmediateTransaction` is `useWriterConnection { it.immediateTransaction { } }` and replaces `db.withTransaction
  { }` in `applyCopy` and `undoCopyImport`; `localTablesChanged()` is `invalidationTracker.createFlow("houses",
  "visits", "photos")`. That call was already the common `createFlow` (an `expect` member in 2.8.5), so the flow is
  unchanged; only the transactions were Android API.
- **Mappers**: `Mappers.kt` and `ExportMappers.kt` moved to `:shared` commonMain (package kept;
  `System.currentTimeMillis` became `IsoTime.nowMillis()`, the same wall clock). `Place`, the geocoder's result, moved
  with `HouseFormRules`.
- **`:ui`**: `CompareScreen` in commonMain, given the live houses (null until Room answers) and the visit counts;
  `:app`'s `ui/CompareTab.kt` keeps `CompareScreen(onOpenHouse, onOpenMap)` and collects both with
  `collectAsStateWithLifecycle`, as before, until CMP-5 brings the common lifecycle library. (Not `CompareScreen.kt`:
  both files would compile to the JVM class `app.doorprints.ui.CompareScreenKt`, and the screenshot test caught the
  clash as a `NoSuchMethodError`.) `String.format` became `formatPositional`, which fills `%1$s` and `%1$d` as Compose
  resources do. Compare's labels come from Compose resources: the 10 `check_*` strings were added in the four
  languages with the Android copies' text (`StringParityTest` compares them), `ChecklistResources`, `labelResource`
  and `glyph` in `:ui` `ModelLabels.kt`. `HouseFormRules.kt` moved (now public). `offerDeletedHouseUndo(repo, …)` is
  in `:ui` over the common `Repository`, and `:app`'s `DeletedHouses.kt` is gone.
- **Tests moved**: `SyncHealthTest` and `ExportGrantsTest` to `:shared` commonTest (**S4b-BL-28**),
  `HouseFormRulesTest` to `:ui` commonTest (its location prompt case, which tests `:app`'s `canAskAgain`, went to
  `LocationAccessTest`).

**How the transaction semantics were checked.** (1) From the sources of Room 2.8.5 and `sqlite-framework` 2.6.2: with no
driver Room runs in compatibility mode; `useWriterConnection` adds the external-operation marker, so
`compatTransactionCoroutineExecute` runs the block through `withTransactionContext` on Room's transaction thread, as
`withTransaction` did; `BEGIN IMMEDIATE TRANSACTION` is turned into
`SupportSQLiteDatabase.beginTransactionNonExclusive()`, which is what `withTransaction` called in WAL mode (the
framework's default); without WAL (low-RAM devices) it called `beginTransaction()` (EXCLUSIVE), but the app's one
process then has one connection, so no other reader existed either way. A DAO call inside sees the open transaction and
nests (another `BEGIN IMMEDIATE`, a nested framework transaction, the same as under `withTransaction`); a throw anywhere
...
**Left in `:app`, and why:** `AndroidRepository` (photo files and decoding, WorkManager, the `HttpClient`; S4b-BL-32);
`CopyUndo`, which only its undo uses; the Android copies of the status and checklist labels (`labelRes`,
`ChecklistLabels`, `check_*`), still read by the house form, the list and the Hunt notification (**S4b-BL-31**); the
...
`:ui:compileTestKotlinIosSimulatorArm64` pass. `shared-ios.yml` on macOS stays the check of record.

**Not verified:** the emulator smoke tests (TC-I-35) run in CI on the pull request; Room on iOS (S4b-BL-24).
...
### 13.9 CMP-5, CMP-6 and CMP-7 (phases 5 to 7), one combined change, done in code (PR #24)

**What was done** (branch `claude/doorprints-dev-continue-fzcge2`, on `main` at `f8dd6f9`, where PR #23 (CMP-4 P4c)
was merged; PR #24, open, the owner merges). The owner asked for CMP-5, CMP-6 and CMP-7 at once, tested together
("After CMP4 is done, make all CMP-5, CMP-6 and CMP-7 changes at once and then test them together so that time can be
saved"), and in the same pull request the web backlog ("Complete Web backlog fixes (4 items) now along with other
Android changes of CMP 5-6-7 as they are fixes and also finish Shared items that also change the web app (6 marked
"Both") now"), and the phone display issues ("The web app has display issues when opened on a mobile browser, check
and fix those as well"). Commits, in order:

| Step | Commits | What |
|---|---|---|
| Emulator on three levels | `e5e43af` | `android-emulator.yml` on API 26, 34 and 36 (owner: "extend the emulator job in GitHub Actions to two or three Android versions, e.g. the oldest supported and the newest") |
| CMP-5 | `84fa735`, `13091ac` | Navigation, view models and five screens in `:ui` commonMain; tests |
| OpenGL ES fix | `b47a67a` | The API 26 job found the map crashing without Vulkan: MapLibre's OpenGL ES build ([03](03-design.md) ADR-22) |
| CMP-6 | `f7e9ecd` (P6a), `7e73411` (P6b), `cff4a63` | The house form; Export, Import and `ImportViewModel`; a carried-in fix and parity tests |
| Web backlog | `a9c28e8`, `95256e0`, `92de59e` | S4b-BL-1, -2, -6, -7; the web halves of S4b-BL-13 and -20 |
| CMP-7 | `e2b163c` | The Map chrome, `PlatformMap`, `applyIndiaView` over `StyleOps` |
...
- **CMP-5** (step 1). JetBrains navigation-compose 2.9.2 (androidx navigation 2.9.7 on Android) and lifecycle 2.11.0
  ...
  `SettingsServices`, the language change), `PlatformServices` grown by the location permission state, the app's
  settings and the notification check, `expect` permission requests. `CopyRecord` and `CopyUndoOutcome` moved to
  `:shared`. `:app` keeps `MainActivity`'s intent parsing (`Notifications.SCREEN_*` equal `Routes.*`) and gains
  `AndroidAppServices`; `CompareTab.kt` went (the wrapper part of S4b-BL-34). The house list reads its status labels
  from Compose resources (part of S4b-BL-31).
- **CMP-6** (step 2). P6a: `HouseEditScreen` with `PhotoDeleteViewModel`; the camera, the photo picker, the geocoder,
  adding a photo and the image model behind `HouseFormServices`; `PlatformServices.dial` and `openUrl`; `expect
  PlatformBackHandler`; Coil 3 in `:ui`. P6b: `ExportScreen`, `ImportScreen` and `ImportViewModel`; the workers,
  pickers, staging and notifications behind `ExportServices` and `ImportServices` (`AndroidBackupServices` in `:app`),
  runs seen as `ExportRun` and `ImportRun`; `ExportProblem` (codes) and `ImportFlow.kt` (`ImportCheck`,
  `ImportStaging`, `ImportRequest`, `ImportStart`) in `:shared`; the result texts and failure reasons copied to Compose
  resources (S4b-BL-35); the form's status and checklist labels from Compose resources, `ChecklistLabels` and the
  Android `check_*` gone (S4b-BL-31). `RootScreens` then held only the Map. Carried-in fix from the PR #23 review
  (`cff4a63`): a copy import cancelled after its commit no longer deletes the photo files of committed rows.
- **Web backlog** (in parallel): S4b-BL-1 (the sync keeps its failure card and the banner line through a retry),
  S4b-BL-2 (the house page's four cards), S4b-BL-6 (a visible invalid field), S4b-BL-7 (TestBed specs), the web half
  of S4b-BL-20 (a reset found from a push answer) and of S4b-BL-13 (comments) (`web/README.md` row of 2026-09-24).
- **CMP-7** (step 3). The `MapScreen` chrome in commonMain; `expect fun PlatformMap(…)` with `MapControl`, `MapEvents`,
  `MapAttribution` and `CameraSpot`; `AppServices.mapScreen: MapServices` (Hunt mode, the notification prompt, motion
  and font settings); the Android actual is the same `MapView` in `AndroidView` (an empty box in inspection mode, as
  MapLibre cannot load on the JVM); the iOS actual is an empty box until CMP-8. `RootScreens`, `AndroidRootScreens` and
  `AppRepository` went; `HuntState` moved to `:shared`. India's view is `applyIndiaView(ops: StyleOps)` in
  `IndiaViewOps.kt` (the old `IndiaView.kt`'s steps, order, fallbacks and warning texts) over `MapLibreStyleOps`.
- **Shared items**: S4b-BL-13 (Android KDoc: both renderers skip a minzoom 5 layer in a zoom 0-4 tile, the guards are
  defence in depth), S4b-BL-17 (hand-overs; Doklam unchanged on purpose), S4b-BL-20 complete (`maxSyncVersion` in
  `GET /api/stats`, used by Android and the web; [08](08-operations-runbook.md) §11.1), S4b-BL-12 (the held-areas
  rule), S4b-BL-9 checked (no rebuild needed), S4b-BL-10 researched (no redistributable source). ADR-22 as updated.
- **Phone display** (the owner's report, with a screenshot of the Map page on a 384 px phone at a larger text size):
  the map sized to the visible page less the list's heading and counters, the credits folded after 5 s, the legend on
  its own row when needed, capped banners, the bar stepping aside for the keyboard, long words wrapped, the skip link
  hidden by its own height, 44 px targets ([05](05-ux-accessibility-i18n.md) §5.2). The missing map labels seen
  during the work were the local test server's: it served the module worker `.mjs` as `application/octet-stream`.

**Differences from the plan.** (1) One pull request for three phases and the backlog items, as the owner asked.
(2) CMP-7 did not write iOS's `UIKitView` around `MLNMapView`: the iOS actual is an empty box, and the MapLibre iOS
view, its `StyleOps` and `MapServices` go to CMP-8 (S4b-BL-43). (3) CMP-6 used Coil 3 for the photos (not in the
plan) because Android's image loading is not common. (4) The owner deferred the TC-M-25 device run to "after CMP-8
as real world testing after completion of development"; until then the emulator job runs on three levels.
...
`ExportBuilder.defaults`; S4b-BL-39), `CurrentLocation.kt` (Play services), `MainActivity`'s intent parsing,
`AndroidRepository` (S4b-BL-32). **New backlog:** S4b-BL-35 (done here) to S4b-BL-50 (§12.7).

**Verified, and how.** Before each commit, locally: `assembleDebug testDebugUnitTest :shared:testAndroidHostTest
:ui:testAndroidHostTest`, both metadata compiles, `-Proborazzi.test.verify=true` and `assembleDebugAndroidTest`; the
iOS set with `-Pkotlin.native.enableKlibsCrossCompilation=true` (`:shared` and `:ui` iOS klibs, `:ui` test klib).
Counts at `18b631e`: `:app` 180 JVM test runs (177 at `f8dd6f9`), `:ui` 110 (67), `:shared` 200 (196); the 64
screenshots match after every step (none re-recorded). Web: 41 files and 514 tests pass, `ng build` passes (two
NG8102 warnings known before). Backend: `mvn -B -ntp verify` on JDK 25 against the `backend/db` image, 287 tests,
1 skipped. CI (Android, the emulator on API 26, 34 and 36, Shared-iOS, Backend, Web, Security) green on the pushed
commits; the last emulator run is on `18b631e` (run 35992367819); `afab9d4` and `2d28529` change only the web,
`tools/live-ui` and docs. `tools/live-ui` against a local build of `2d28529`: exit 0, mobile 324 and map 23 checks pass
([06](06-test-plan.md) TC-M-26). New tests: [06](06-test-plan.md) TC-U-68 to TC-U-77, TC-I-36.

**Not verified:** TC-M-25 on a device (deferred by the owner to after CMP-8) and on the live site (after the merge and
deploy); map labels on a device (the emulator lays them out but does not draw them, S4b-BL-48); the iOS actuals
(compile-only; S4b-BL-36, -39, -40, -43); the live UI test on the live site (after the merge, N8).

**Docs.** [01](01-requirements.md) v0.32, [02](02-threat-model.md) v0.35, [03](03-design.md) v0.31,
[05](05-ux-accessibility-i18n.md) v0.24, [06](06-test-plan.md) v0.45, [07](07-secure-build-and-deploy.md) v0.39,
[08](08-operations-runbook.md) v0.18, this section (v0.51), [14](14-lead-backlog-and-handoff.md) v0.18, the docs index
