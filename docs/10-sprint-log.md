...
| ID | Candidate | Why | Teams (proposed) | Notes for planning |
|---|---|---|---|---|
| C-22 | **CodeQL** code scanning (Java/Kotlin, JavaScript/TypeScript) and SARIF upload of Semgrep to the Security tab | Free now that the repository is public; better SAST coverage | DevOps, Security | **Landed in Sprint 4a** (§11.1): `.github/workflows/codeql.yml` analyses `java-kotlin` and `javascript-typescript` with `build-mode: none`, on push to `main`, weekly and manually ([07](07-secure-build-and-deploy.md) §1). Follow-ups, still open: (a) **Android Kotlin is not analysed** — buildless Kotlin needs a real Gradle run, so it stays covered by Semgrep; (b) the **Semgrep SARIF is still only an artifact**, not uploaded to the Security tab; (c) no run yet (nothing in this change set is pushed, §11.2) |
...
round at the coordinator's request) carries the 1.32 and 1.33 changes. Where each landed:
...
### 12.5 Owner decisions of 2026-09-23: security split, release guard, review efficiency, process improvements and the first release's Definition of Done
...
2. **Every workflow that runs on the merge commit is green**: `backend.yml`, `web.yml`, `android.yml`, `security.yml`
   ...
   compile runs), and `codeql.yml`, which runs on every push to `main`. `ai-evals.yml` is manual and not part of
...
in `web/README.md` (*Pre-deploy close-out, web buddy pre-review*). Duplicates are merged: the same `sync.service.ts`
item was raised three times and is one ticket.
...
**Backlog tickets (Sprint 4b candidates).**
...
| ~~(W2)~~ | Web | ~~Plan's submit focuses the start latitude: already a carried minor in §11.7~~ **Done by Web in the buddy pre-review, awaiting review** (`pages/plan/start-field.ts`, `start-field.spec.ts`; §11.7) | As in §11.7 | Web |

**`NEW RULE:` candidates for the playbooks.** Items (b) to (e) are now in the design and UX self-check. (a), (f), (g)
and (h) are recorded here until a reviewer adopts them. (f) and (g) are the coordinator's proposals from this round's
backlog; (h) is from the round 1 review of this close-out (v0.28).
...
| (f) | R6/R17: every async callback that can navigate, announce or change a map (geolocation, HTTP, timers) first checks that its page still exists | Delivery coordinator | Candidate for the Design Director and the UX lead. Both first cases are done by Web in the working tree, awaiting review: the map and Plan guard above and S4b-BL-5 |
