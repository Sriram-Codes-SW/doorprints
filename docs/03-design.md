...
| `:ui` (ADR-23, since `be86f50`) | Compose Multiplatform UI code that is not tied to Android: the theme (`DoorprintsTheme`, language lookup `expect fun uiLanguage()`), list rows, the server status and its rules, the Map and India view rules, `ButtonLabel`, `ResultTone`, `LocationFix`; `ServerStatusTest` in `commonTest`. Kotlin package `app.doorprints.ui`, Android namespace `app.doorprints.ui` | Same `commonMain` rules as `:shared`; `api(project(":shared"))` and the Compose Multiplatform libraries (1.12.1; material3 1.9.0). The screens move here phase by phase ([`android/ui/README.md`](../android/ui/README.md) §6) |
