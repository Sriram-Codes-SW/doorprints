...
| `commonMain` `app.doorprints.data` (CMP-4 P4b) | `SettingsStore` on a `DataStore<Preferences>` (`datastore-preferences-core` 1.1.7; `FILE_NAME` `settings` and every key name kept), `AppSettings`, `ResultMarks`, `SyncHealth`, the `SecretStore` interface (`get`, `put`, `clear`), `ServerUrl` with `UriParts`; in `app.doorprints.export` the export grant list | Stored names never change (ADR-24); the key never enters common code except through `SecretStore` |
