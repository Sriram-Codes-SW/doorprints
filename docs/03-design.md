...
```mermaid
flowchart TB
    ma["MainActivity<br/>deep links from notifications"] --> root["DoorprintsRoot<br/>NavHost + bottom bar"]
...
| `DoorprintsRoot` | `ui/Root.kt` | Tabs: Map, Houses, Compare, (Assistant when AI is on), Settings. Routes `house/{id}` and `new?lat&lon&visitId`. Wraps everything in `DoorprintsTheme` (`ui/Theme.kt`: web tokens, dark scheme). |
...
| `HouseListScreen`, `CompareScreen` | `ui/*.kt`; Compare in `:ui` commonMain since CMP-4 P4c | List with search/filter/sort. Compare up to 4 (shortlisted first). |
...
flowchart LR
    ...
    subgraph uim[":ui (Compose Multiplatform, ADR-23)"]
        uicm["commonMain: Theme, Rows, ServerStatus,<br/>MapRules, IndiaViewRules, Buttons,<br/>Format, CompareScreen,<br/>HouseFormRules"]
