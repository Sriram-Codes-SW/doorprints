...
| 0.20 | 2026-09-24 | Claude (Code), Docs team | **ADR-22** re-synced with branch `fix/india-boundary-lines` (commit `5af2f4d`; S4b-BL-11, S4b-BL-15, S4b-BL-16). **Data:** new file (418 518 bytes, sha256 `8ef39ebc…42e8`), three kinds: `world` now also holds the 11 **shared stretches** of India's outline along which the tiles draw India's border themselves (drawn below zoom 5 only), `claim` the rest with a connector of at most 7 km at each hand-over, new `state` (the Assam-Arunachal Pradesh line, Natural Earth 1:10m admin-1); how `find_shared_stretches.py` finds the stretches. **Rule 3:** new layer `in-boundary-state` from zoom 5, directly above `boundary_3` and drawn like it. **Consequences:** the two close lines and the missing state line are gone; new: the hand-over steps and the 2 km loop in Sikkim, and the shared stretches without a line at zoom 5+ while tiles load or offline. |
