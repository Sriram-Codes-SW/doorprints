...
| 0.23 | 2026-09-24 | Claude (Code), Docs team | **ADR-23** guard rails: each phase is verified by the JVM screenshot tests ([06](06-test-plan.md) TC-U-60: every screen but the Map, en/hi/ta/te, light and dark, 64 reference images checked by `android.yml`) and the instrumented smoke tests on an emulator ([06](06-test-plan.md) TC-I-35, `android-emulator.yml`); a phase that changes a screen on purpose re-records its images and says so. Owner request of 2026-09-24, commit `afe4064`, ticket CMP-0 ([10](10-sprint-log.md) §13.3). |
