...
- **Compose Multiplatform track** (owner request of 2026-09-24, [03](03-design.md) ADR-23, [10](10-sprint-log.md) §13): **CMP-1 done** in commit `be86f50` on branch `claude/doorprints-dev-continue-fzcge2` (new `:ui` module, [`android/ui/README.md`](../android/ui/README.md); theme and pure UI code moved; no visual change). Local build and tests pass; the iOS compile of `:ui` is verified by CI on macOS only (`shared-ios.yml`) and is pending. Not merged: the owner merges.
