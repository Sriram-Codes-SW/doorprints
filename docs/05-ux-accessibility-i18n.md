...
**Android** has no CSS, and Material 3's type scale sets a fixed line height (bodySmall 12/16 sp, bodyMedium 14/20) that overrides the taller spacing of the Indic fallback fonts, so the subscript conjuncts of one Tamil or Telugu line meet the vowel signs of the next. `HouseHuntTheme` (`ui/Theme.kt`) therefore switches to **`IndicTypography`** when the configuration's locale is `hi`, `ta` or `te` (the per-app language on every supported API level). It keeps M3's sizes and weights, sets **letter spacing 0** (Latin tracking pulls conjuncts apart) and these line heights, about 1.6–1.7× — the Android counterpart of `--leading: 1.7`:
...
| `secondary` | `--star` | `#A86A00` | `#F2B84B` | Star colour only (the stars themselves use `LocalHouseHuntColors.star`) |
...
added or changed. Files are under `android/app/src/main/java/com/househunt/app/ui/`.
...
|---|---|
| **India's outline** | One solid line, in the base map's own country-line colour, width and opacity, with round joins and caps, around all of Jammu and Kashmir and Ladakh (including the areas the tiles call Azad Kashmir, Gilgit-Baltistan, the Shaksgam valley and Aksai Chin) and along the Himachal Pradesh and Uttarakhand border with Tibet and Nepal (Kalapani inside India), the Sikkim border with Tibet (the Doklam tri-junction) and around Arunachal Pradesh: Natural Earth's India point-of-view outline in those four areas. At every zoom. |
| **No de facto or claim line** | No Line of Control, no Line of Actual Control and no other de facto or claim line, dashed or solid, at any zoom, also while closer tiles are still loading and offline (Android; the web has an open gap here, see *Offline and errors*); no line between Pakistan-occupied Kashmir and the rest of Jammu and Kashmir, and none across Aksai Chin or Arunachal Pradesh; and the tiles' Pakistan-China line at the Khunjerab pass is not drawn (India's own outline is the line there). India's outline is not the only line in the four areas, though: see the next row. Inside the outline, in the tiles checked (zoom 5 and 6, planet of 2026-09-13), only Indian internal lines (for example Jammu and Kashmir-Ladakh, dashed) are drawn, apart from a few short stretches along the outline itself: the lines of Pakistani and Chinese administrative units there are flagged disputed, so the base style itself hides them (zoom 7 and above: TC-M-25). |
| **Country lines** | Below zoom 5 every country line comes from the bundled Natural Earth file (1:50m, India's classification), so the world view looks much as before except near India. From zoom 5 the tiles' own country lines are drawn everywhere, only those that carry a country code (so never the zoom 0-4 tile MapLibre shows while a closer tile loads) and never the Pakistan-China line. The four areas are not cut out: there the tiles' non-disputed lines along the same borders as India's outline are drawn too, which means Himachal Pradesh and Uttarakhand with Tibet (the stretches the tiles do not flag as disputed) and with Nepal near Kalapani, Sikkim with Nepal, Tibet and Bhutan, and Arunachal Pradesh with Bhutan and Myanmar. The two sources can differ by up to about a kilometre, so when zoomed in the user can see two close, nearly parallel solid lines there (known limit, [10](10-sprint-log.md) S4b-BL-11). Against China in Ladakh and Arunachal Pradesh, and along the Jammu International Border, the tiles' lines are disputed and hidden, so India's outline is the only line. |
...
| **Offline and errors** | The outline is bundled (web: precached with the app; Android: an asset), so it needs no network of its own and appears whenever the base style loads, including after the web's offline retry. On Android (and on the web once the gap below is closed), offline at zoom 5 or more with only the country view's tiles cached, no tile country line is drawn (only India's outline) until closer tiles load. If the base style has changed and a layer is missing, the map still works and the outline is still drawn; the app logs a warning (web console, Android `Log.w`) and shows the user nothing. **Open parity gap (web, not deliberate):** the web's country-line rule has no country-code guard yet, so while a zoom 5+ tile loads (for example after *Zoom in* from the country view on a slow network) or is not cached offline, the web can show the zoom 4 tile's Pakistan line through Kashmir and the line across Arunachal Pradesh at zoom 5 or more. Handed to Web ([11](11-feature-parity-and-export-spec.md) §10); until it is closed the web fails TC-M-25. |
...
Earth data, so when zoomed far into the mountains it can be about a kilometre off the tiles' rivers and roads, and
along the non-disputed borders in the four areas it can show as a second line beside the tiles' own (*Country lines*).
...
inside the outline), on the live site and on an Android device. This section was last compared with the code as of
2026-09-23 19:56 UTC (2026-09-24 01:26 IST).
...
ships (`android/shared/README.md` 1.10 to 1.14, strings in `values*/strings.xml`).
...
grep -n "%d" android/app/src/main/res/values/strings.xml       # R2: each hit must be inside <plurals>
grep -rLn "imePadding\|WindowInsets.ime" android/app/src/main/java/com/househunt/app/ui/*Screen.kt           # R5: screens with fields
# Web
