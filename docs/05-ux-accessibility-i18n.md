...
| **No de facto or claim line** | No Line of Control, no Line of Actual Control and no other de facto or claim line, dashed or solid, at any zoom, also while closer tiles are still loading and offline, on both apps; no line between Pakistan-occupied Kashmir and the rest of Jammu and Kashmir, and none across Aksai Chin or Arunachal Pradesh; and the tiles' Pakistan-China line at the Khunjerab pass is not drawn (India's own outline is the line there). India's outline is not the only line in the four areas, though: see the next row. Inside the outline, in the tiles checked (zoom 5 and 6, planet of 2026-09-13), only Indian internal lines (for example Jammu and Kashmir-Ladakh, dashed) are drawn, apart from a few short stretches along the outline itself: the lines of Pakistani and Chinese administrative units there are flagged disputed, so the base style itself hides them (zoom 7 and above: TC-M-25). |
| **Country lines** | Below zoom 5 every country line comes from the bundled Natural Earth file (1:50m, India's classification), so the world view looks much as before except near India. From zoom 5 the tiles' own country lines are drawn everywhere, only those from a zoom 5 or closer tile that carry a country code (so never a line of the zoom 0-4 tile shown while a closer tile loads) and never the Pakistan-China line. The four areas are not cut out: in some places the tiles' non-disputed lines along the same borders as India's outline are drawn too, namely the Wakhan (with Afghanistan), Himachal Pradesh and Uttarakhand with Tibet (the stretches the tiles do not flag as disputed) and with Nepal near Kalapani and Dharchula, Sikkim with Nepal, Tibet and Bhutan, Bhutan's south-east corner and Myanmar south of 26.65 N. The two lines are a median 1.5-2.8 km apart there, at most 5.3 km, so when zoomed in the user can see two close, nearly parallel solid lines (known limit, [10](10-sprint-log.md) S4b-BL-11 and S4b-BL-16). Along Arunachal Pradesh's borders with Bhutan and with Myanmar, against China in Ladakh and Arunachal Pradesh, and along the Jammu International Border at Sialkot, the tiles' lines are disputed and hidden, so India's outline is the only line; a second line there is a failure of [06](06-test-plan.md) TC-M-25. |
| **State lines** | State and other administrative lines (dashed, the base map's own style) from zoom 5, as before, now only from a zoom 5 or closer tile. **Known limit:** from zoom 5 the Assam-Arunachal Pradesh state line is not drawn on either app: the tiles carry it as a disputed line claimed by China, and the base style never draws a disputed state line. Arunachal Pradesh keeps its external outline ([03](03-design.md) ADR-22 *Consequences*; backlog [10](10-sprint-log.md) S4b-BL-15). |
...
| **Offline and errors** | The outline is bundled (web: precached with the app; Android: an asset), so it needs no network of its own and appears whenever the base style loads, including after the web's offline retry. On both apps, offline at zoom 5 or more with only the country view's tiles cached, no tile country or state line is drawn (only India's outline) until closer tiles load; no line is better than a wrong line through Indian territory. If the base style has changed and a layer is missing, the map still works and the outline is still drawn; the app logs a warning (web console, Android `Log.w`) and shows the user nothing. |
...
percentile, so when zoomed far into the mountains it may not line up with the tiles' rivers and roads; along some
non-disputed borders in the four areas it shows as a second line beside the tiles' own (*Country lines*); and the
Assam-Arunachal Pradesh state line is not drawn from zoom 5 (*State lines*).
...
inside the outline; a second line along Arunachal-Bhutan, Arunachal-Myanmar or Jammu-Sialkot fails), on the live
site and on an Android device. This section was last compared with the code at HEAD `3ad2b58` (2026-09-24).
