...
| 0.18 | 2026-09-24 | Claude (Code), Docs team | **§7.3** re-synced with branch `fix/india-boundary-lines` (commit `5af2f4d`; [03](03-design.md) ADR-22 v0.20). *India's outline*: along the shared stretches it draws below zoom 5 only and the tiles' line takes over from zoom 5, with a connector of at most 7 km at each hand-over. *Country lines*: one line everywhere, no more two close lines (S4b-BL-11, S4b-BL-16). *State lines*: the Assam-Arunachal Pradesh line is drawn from zoom 5 like the other state lines (S4b-BL-15). *Offline and errors* and the known limits: the shared stretches show no line at zoom 5+ until tiles are cached; the hand-over steps and the 2 km loop in Sikkim. |
...
|---|---|
| **India's outline** | One solid line, in the base map's own country-line colour, width and opacity, with round joins and caps, around all of Jammu and Kashmir and Ladakh (including the areas the tiles call Azad Kashmir, Gilgit-Baltistan, the Shaksgam valley and Aksai Chin) and along the Himachal Pradesh and Uttarakhand border with Tibet and Nepal (Kalapani inside India), the Sikkim border with Tibet (the Doklam tri-junction) and around Arunachal Pradesh: Natural Earth's India point-of-view outline in those four areas, at every zoom, except along the stretches where the tiles draw India's border themselves (Himachal Pradesh and Uttarakhand with Tibet, Uttarakhand with Tibet and Nepal near Kalapani, Sikkim with Nepal, Tibet and Bhutan, Bhutan's south-east corner, Myanmar south of about 26.65 N and the Wakhan): there the outline draws below zoom 5 only, and from zoom 5 the tiles' own, more precise line is the border. At each hand-over a short straight connector (at most 7 km) joins the two, so the border is one continuous line. |
...
| **Country lines** | Below zoom 5 every country line comes from the bundled Natural Earth file (1:50m, India's classification), so the world view looks much as before except near India. From zoom 5 the tiles' own country lines are drawn everywhere, only those from a zoom 5 or closer tile that carry a country code (so never a line of the zoom 0-4 tile shown while a closer tile loads) and never the Pakistan-China line. The four areas are not cut out, but along the stretches where the tiles draw India's border themselves (*India's outline*) India's outline stops at zoom 5, so the two close lines shown there before branch `fix/india-boundary-lines` are gone ([10](10-sprint-log.md) S4b-BL-11 and S4b-BL-16, done). Along Arunachal Pradesh's borders with Bhutan and with Myanmar, against China in Ladakh and Arunachal Pradesh, along the Jammu International Border at Sialkot and on the parts of the middle sector and Sikkim where the tiles have only disputed lines, those lines are hidden, so India's outline is the only line. A second line beside the border anywhere is a failure of [06](06-test-plan.md) TC-M-25. |
...
| **Offline and errors** | The outline is bundled (web: precached with the app; Android: an asset), so it needs no network of its own and appears whenever the base style loads, including after the web's offline retry. On both apps, offline at zoom 5 or more with only the country view's tiles cached, no tile country or state line is drawn (only India's outline and the Assam-Arunachal Pradesh state line) until closer tiles load, so the stretches where the tiles' line is the border show no line at zoom 5 or more until then; no line is better than a wrong line through Indian territory. If the base style has changed and a layer is missing, the map still works and the outline is still drawn; the app logs a warning (web console, Android `Log.w`) and shows the user nothing. |
...
never in a city); at street zoom each hand-over to the tiles' line shows as a small step (at most 7 km), and in Sikkim
one hand-over makes a small closed loop of about 2 km; and while closer tiles load, or offline without them, the
...
on the live site and on an Android device. This section was last compared with the code at `5af2f4d` (branch
