...
| AI evals | Grounding, citations, injection, extraction accuracy, planner validity | Golden set (`docs/ai/evals/golden-set.json` v0.3, with thresholds) + harness in `backend/src/test/java/com/househunt/ai/eval/` (AI team): `EvalScorerTest` (pure scoring, every build) and `GoldenSetEvalTest` (real model, tag `llm-eval`, skipped unless `AI_API_KEY` is set) | Scoring: every push. Model evals: manually through `.github/workflows/ai-evals.yml` (Actions > AI evals > Run workflow; needs the `AI_API_KEY` repository secret; scorecard in the job summary and the `ai-eval-report` artifact), before enabling AI by default and on model/prompt change | Harness and workflow built in Sprint 2; first real run 35720654442 recorded ([10](10-sprint-log.md) §5.3, [ai-design](ai/ai-design.md) §8.5): 12/13, E-03 open. Details in [ai/](ai/) |
...
| TC-AI-10 after the Sprint 3 embedding and scorecard fixes | First real scorecard recorded ([10](10-sprint-log.md) §5.3, Actions run 35720654442): all 13 cases ran, 12/13 passed, `citationPrecision` 0.86 < 0.90; E-01 and E-02 confirmed fixed. The golden-set fixtures hold no contact data, so the run does not test contact redaction against the provider (TC-AI-15 covers it without a model) | E-03 (AI team: `allowedCitations` in the golden set and the Ask prompt tweak), then re-run `ai-evals.yml`; Sprint 4 candidate C-21 (fixture with a contact plus a `mustNotContain` check) |
