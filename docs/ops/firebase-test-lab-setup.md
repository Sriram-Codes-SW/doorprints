...
| 0.5 | 2026-09-24 | Claude (Code), Docs team | PR #18 round 7: the bucket is in `us-central1` (the only way to stay inside Always Free); the main-only and emulator sentences are back at the end of the Status paragraph. |
...
already) and `FTL_RESULTS_BUCKET` and the repository secrets `FTL_WIF_PROVIDER` and `FTL_SA_EMAIL` are all set. It
runs only from `main` (a push or a manual run). The emulator job in the same workflow runs without any of this.
...
link billing with a ₹0 budget alert, or go back to Test Lab's default bucket).
...
3. Create the results bucket, for example `doorprints-test-lab-results`, in `us-central1` (the only way to stay
   inside Always Free, option (b); not the project's region), with uniform access and a lifecycle rule that deletes
   objects after 30 days. Grant `ftl-runner` Storage Object Admin on that bucket only, plus Storage Legacy Bucket
   Reader there if the first run fails on `storage.buckets.get`.
