...
```mermaid
flowchart LR
    R["Repository.sync"] --> RI["RetryInterceptor<br/>idempotent only, 3 attempts,<br/>full-jitter backoff, Retry-After"]
    RI --> OK["OkHttp shared pool<br/>TLS 1.2+, system CAs only,<br/>no redirects, gzip"]
