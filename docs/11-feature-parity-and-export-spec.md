...
| Map boundaries (India) | `applyIndiaView(style)` (`ui/IndiaView.kt`, rules as data in `ui/IndiaViewRules.kt`) from `MapScreen.loadStyle`, before the house layers, on every style load; data `asset://geo/in-boundaries.geojson`; a skipped rule is `Log.w` (tag `IndiaView`). No "Natural Earth" attribution credit (MapLibre Android 13.6.1 has no API for a runtime source's attribution; public domain, so none is required) | `applyIndiaBoundaries` (`shared/india-boundaries.ts`) registered by `createMlMap` on every `style.load` (Map, Plan, house pages; also after `watchMapStyle`'s offline retry); data `geo/in-boundaries.geojson` against the base href, precached; a skipped rule is `console.warn`; "Natural Earth" in the attribution. **The same on both** (compared rule by rule with the code as of 2026-09-23 19:56 UTC, 2026-09-24 01:26 IST): the same file (sha256 `700646ea…4954`, byte identity held by `IndiaBoundaryDataTest`), the same ids (`in-boundaries`, `in-boundary-world`, `in-boundary-claim`), rules 1, 3, 4 and 5, the PAK/CHN part of rule 2 and its minzoom `max(own, 5)`, the name lists, the placement and fallback paint, the deprecated-syntax safety net. The world lines hand over at exactly zoom 5.0 on both: the web's maxzoom 5 is exclusive in maplibre-gl, and Android's is `Math.nextDown(5f)` because maplibre-native includes both ends of a zoom range; the same behaviour, not a difference. **Deliberate difference:** the "Natural Earth" credit, web only. **Open parity gap (not deliberate; Web, `android/shared/README.md` §9 item 36):** Android's rule 2 also requires an adm0 side (`["any", ["has", "adm0_l"], ["has", "adm0_r"]]`, in both syntaxes, `IndiaViewRules.COUNTRY_LINE_EXTRA_FILTER`), the web's (`NOT_PAKISTAN_CHINA` in `india-boundaries.ts`) does not, so while a zoom 5+ tile loads or is not cached offline the web can draw the zoom 4 tile's ISO-view line through Kashmir and across Arunachal Pradesh at zoom 5 or more ([06](06-test-plan.md) TC-M-25 steps 7 to 9). When Web adds the guard and its spec cases (TC-U-54), this row says "the same five rules" again (D-26, [03](03-design.md) ADR-22, [05](05-ux-accessibility-i18n.md) §7.3) | 4a fix (2026-09-24); web adm0 guard open |
