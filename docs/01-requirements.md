...
| 0.24 | 2026-09-24 | Claude (Code), Docs team | **FR-098** known limits re-synced with branch `fix/india-boundary-lines` (commit `5af2f4d`): the Assam-Arunachal Pradesh state line is drawn from zoom 5 and the two close lines are gone (S4b-BL-11, -15, -16); the remaining limits are the hand-over steps, the 2 km loop in Sikkim and no line on the shared stretches at zoom 5+ while tiles load or offline. |
