...
| ID | Requirement | Pri | Status |
|---|---|---|---|
| AI-001 | Every AI feature is disabled unless the operator configures a provider (for example the Gemini API free tier or a local Ollama). When disabled, AI endpoints return 404 and both clients hide AI entry points (`GET /api/ai/status`). | M | Impl |
...
Multi-tenant accounts, sharing links, iOS, push notifications from the server, scraping property portals, payments, legal/title verification, turn-by-turn navigation, offline tile packs (Could, later).
