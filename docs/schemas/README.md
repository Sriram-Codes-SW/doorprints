...
| Where | File |
|---|---|
| Server | `backend/src/main/java/com/househunt/backup/` (`BackupFormat`, `BackupData`, `BackupHouse`, `BackupVisit`, `BackupPhoto`) |
| Android | `android/shared/src/commonMain/kotlin/com/househunt/shared/export/Backup.kt` and `ExportModel.kt` |
...
- **the parsed-JSON golden of the server and of Android.** Put the same rows in, export, parse both sides and
  ...
  **as numbers**, so `0` and `0.0` are equal. The server does this in
  ...
  `android/app/src/test/java/com/househunt/app/CanonicalSampleTest.kt` (ticket S4-00/a), which reads this file
