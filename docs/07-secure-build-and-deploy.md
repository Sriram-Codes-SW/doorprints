...
| 0.31 | 2026-09-24 | Claude (Code), Docs team | **Testing the APK and the live web UI** (owner request of 2026-09-24, commit `afe4064`; [06](06-test-plan.md) §16, [10](10-sprint-log.md) §13.3). **§1:** new workflow **`android-emulator.yml`** (same triggers and path filters as `android.yml`): job `emulator` (API 34 emulator, KVM, `connectedDebugAndroidTest`, artifact `android-emulator-results`), jobs `ftl-check` and `firebase-test-lab` (Firebase Test Lab, pushes to `main` and manual runs, keyless). `android.yml` runs its unit tests with `-Proborazzi.test.verify=true` (screenshot tests, TC-U-60) and uploads `app/build/outputs/roborazzi` in `android-reports`. The flowchart, the *Branch runs* table, the workflow table and the Conventions (least privilege: `id-token: write` only on `firebase-test-lab`; pinning of `ReactiveCircus/android-emulator-runner` v2.38.0 and `google-github-actions/setup-gcloud` v3.0.1; timeouts 5 to 45 minutes; artifact retention) are updated. New **§7.2**, the owner's Firebase Test Lab setup (APIs, service account `ftl-runner` with least-privilege roles, the Workload Identity provider's condition and the per-workflow impersonation, the secret `FTL_SA_EMAIL`), with a pointer file [ops/firebase-test-lab-setup.md](ops/firebase-test-lab-setup.md). `tools/live-ui` (the live web UI test, run by the lead after each merge, not by a workflow) is named in §1. |
...
flowchart LR
    ...
    dev --> emu
    subgraph emu["android-emulator.yml (same paths as android.yml)"]
        ...
        e3["main only, push or manual:<br/>ftl-check - three values set?"] --> e4["firebase-test-lab: WIF token<br/>(own provider),<br/>gcloud firebase test android run,<br/>one device"]
    end
...
**What it is for.** `android-emulator.yml` job `firebase-test-lab` runs the smoke tests ([06](06-test-plan.md)
TC-I-35) on one Firebase Test Lab device (default `model=MediumPhone.arm,version=34`) on every push to `main` that
touches `android/**`, and on a manual run from `main`. It uses the free Spark quota: **10 virtual and 5 physical device
runs a day**, one device per run. Owner choice of 2026-09-24 ([10](10-sprint-log.md) §13.3). The workflow's header
points to the short guide [ops/firebase-test-lab-setup.md](ops/firebase-test-lab-setup.md), which points here.

**Status (2026-09-24): not set up.** Until all three values below exist, the job `ftl-check` posts the notice
"Skipped: FIREBASE_PROJECT_ID (variable), FTL_WIF_PROVIDER and FTL_SA_EMAIL (secrets) are not all set" and the Test Lab
job is skipped. `ftl-check` runs only on `refs/heads/main` (a push or a manual run); a run from any other branch
skips both Test Lab jobs. The emulator job does not depend on any of this.

**Design: a provider of its own.** Test Lab uses the Firebase project `doorprints` (Spark plan, no billing account),
but **not** the Hosting deploy's Workload Identity provider: `github-web-deploy` keeps accepting only `web.yml` on
`main` (6.3 step 6), unchanged. A second provider, `github-test-lab`, accepts only `android-emulator.yml` on `main`,
and only identities that come through it can act as the service account `ftl-runner`, which can run Test Lab and
nothing else. The code review asked for this ("Don't widen the Hosting provider").

| Name | Kind | Value | New? |
|---|---|---|---|
| `FIREBASE_PROJECT_ID` | Variable | `doorprints` | No (6.3 step 8) |
| `FTL_WIF_PROVIDER` | Secret | `projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/POOL/providers/github-test-lab` | **Yes** (step 4) |
| `FTL_SA_EMAIL` | Secret | `ftl-runner@doorprints.iam.gserviceaccount.com` | **Yes** (step 5) |
...
**Step 3. Access to the results bucket (least privilege).** Test Lab writes each run's results to a Cloud Storage
bucket that it creates for the project the first time Test Lab is used; its name starts with `test-lab-`.

1. If Menu > **Cloud Storage** > **Buckets** shows no `test-lab-…` bucket yet, run one test from the Firebase console
   first: **Test Lab** > **Run a test** > **Run a Robo test**, with `app-debug.apk` from the `doorprints-debug-apk`
   artifact of the latest **Android** run on `main`, on one virtual device. This uses 1 of the day's 10 virtual runs
   and creates the bucket.
2. Open the bucket > **Permissions** > **GRANT ACCESS**. **New principals**: the `ftl-runner` email. **Role**:
   **Storage Object Admin** (`roles/storage.objectAdmin`). Save.

This is the least-privilege choice: `ftl-runner` can read and write objects in that one bucket and nothing else in
Cloud Storage. The simpler alternative, the project role **Editor** (`roles/editor`) instead of this step, also works
but lets the account change almost anything in the project, including the Hosting site; we advise against it. If the
first Test Lab run fails with a storage permission error, send us the red line; do not add Editor on your own.

**Step 4. A Workload Identity provider for the Test Lab workflow only.**

1. **Pool.** Either reuse the pool `github` (6.3 step 6) or create a new one, for example `github-test-lab`
   (**IAM & Admin** > **Workload Identity Federation** > **+ CREATE POOL**). A new pool is the cleaner choice: see
   item 4 for why.
2. **Provider.** In that pool: **ADD PROVIDER** > **OpenID Connect (OIDC)**, **Provider name** and **Provider ID**
   `github-test-lab`, **Issuer (URL)** `https://token.actions.githubusercontent.com`, **Audiences** *Default
   audience*. **Attribute mapping**: the same five rows as 6.3 step 6.4 (`google.subject` = `assertion.sub`, and
   `attribute.repository`, `attribute.repository_id`, `attribute.ref`, `attribute.workflow_ref` from the claims of the
   same names). **Attribute condition**, as one line, with the repository ID of 6.3 step 5 in place of `REPO_ID`:
   ...
   (`projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/POOL/providers/github-test-lab`): it is the value
   of **`FTL_WIF_PROVIDER`**.
3. **Let this repository act as `ftl-runner`.** Service Accounts > `ftl-runner@…` > **PRINCIPALS WITH ACCESS** >
   **GRANT ACCESS**. **New principals** (one line; your project **number**, the pool from item 1 and the repository
   ID):

   ```
   principalSet://iam.googleapis.com/projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/POOL/attribute.repository_id/REPO_ID
   ```
...
4. **Why a new pool is cleaner.** A `principalSet` names a pool, not a provider. In the pool `github`, the principal
   above would also match tokens from `github-web-deploy` (that is, `web.yml` on `main`), and the existing Hosting
   grant (6.3 step 7, `attribute.repository_id` in the pool `github`) would also match tokens from `github-test-lab`,
   so `android-emulator.yml` on `main` could act as the deploy account. Both workflows run only reviewed code from
   `main`, but a separate pool keeps each workflow to its own account without touching the Hosting setup. If you
   reuse the pool `github`, also do the optional hardening note below.

**Step 5. GitHub secrets.** Repository > **Settings** > **Secrets and variables** > **Actions** > **New repository
secret**, twice:

| Name | Value |
|---|---|
| `FTL_WIF_PROVIDER` | the provider's full name from step 4 item 2 |
| `FTL_SA_EMAIL` | `ftl-runner@doorprints.iam.gserviceaccount.com` |
...
to the run in the Firebase console (Test Lab), where the results are kept. The per-step screenshots are kept only by
...
**Quota and cost.** Each push to `main` that touches `android/**`, and each manual run from `main`, uses one virtual
device run of the 10 a day. Nothing here needs the Blaze plan. To stop Test Lab runs, delete the secret `FTL_SA_EMAIL`:
the job is then skipped again.
...
ever added to that pool (for example if step 4 reused it). The fix is to grant `firebase-hosting-deploy` to
...
- [ ] **S10 Web storage and privacy.** No `innerHTML` or `bypassSecurityTrust*`. The API key is in `sessionStorage` by
  default. "Remove all data" clears IndexedDB, Cache Storage, every `hh.*`/`doorprints.*` key in local and session
...
**Check.** `grep -rn "embed\|ChatClient\|prompt(\|HouseSummary.of\|HouseDetails.of" backend/src/main/java/com/househunt/ai`,
...
**Check.** `jq -r '..|.mustContain?[]?' docs/ai/evals/golden-set.json | grep -F -f - backend/src/main/java/com/househunt/ai/rag/AskPrompts.java`
...
and `grep -rn "FLAG_MUTABLE\|FLAG_IMMUTABLE\|ACCESS_COARSE_LOCATION" android/app/src/main/java`. Run TC-S-12 with `app.doorprints/com.househunt.app.MainActivity`.
