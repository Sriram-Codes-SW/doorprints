...
The workflows live in `.github/workflows/` (F-22 fixed). `backend.yml`, `web.yml`, `android.yml` and `security.yml` run on push and pull request to `main` and on manual dispatch; the security workflow also runs weekly. `ai-evals.yml` is the exception: it is manual only (`workflow_dispatch`) and never runs on push or PR. `shared-ios.yml` (Sprint 3.5) runs on push/PR to `main` only when `android/shared/**`, the root Gradle files or the workflow itself change, and on manual dispatch.
...
| Control | How |
|---|---|
| Least privilege | Top-level `permissions: {}` (no token scopes by default); every job declares `permissions: contents: read` and nothing more, with two reviewed exceptions: `ai-evals.yml` adds `id-token: write` (Workload Identity Federation, manual runs only) and `security.yml` `gradle-dependency-graph` has `contents: write` for the Dependency Submission API (never on pull requests; checkout without persisted credentials); no `pull_request_target` |
| Pinning | GitHub-owned actions by major version tag (`actions/checkout@v7`, `setup-java@v6`, `setup-node@v7`, `upload-artifact@v7`, `gradle/actions/setup-gradle@v6`, latest majors on 2026-09-22); third-party actions by full commit SHA (ZAP; `google-github-actions/auth` at v3.0.0 since the DevSecOps review of `ai-evals.yml` on 2026-09-22), kept current by Dependabot; scanners run as version-pinned container images instead of third-party actions with mutable tags (tag hijacking, T-T5); pin the images by digest once the first run has confirmed the tags |
