...
- **Android: settings, the API key's interface and the URL check are common code in `:shared`**
  ...
  goes only through a new `SecretStore` interface (`get`, `put`, `clear`). Upgraded installs keep their settings and
  saved key: same file (`files/datastore/settings.preferences_pb`, opened in `:app` as the old delegate did), same key
  names, same Keystore alias and `apiKeyEnc` entry (`ApiKeyCipher` unchanged, wrapped by `KeystoreSecretStore`); new
